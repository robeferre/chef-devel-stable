...
#######################################################################
RESPONSE_FILE_PATH = "#{node['bpm8000']['config']['cache']}/#{node['bpm8000']['respfile']['dbexpress']}"
...
if (!(File.exists?(RESPONSE_FILE_PATH)))
   ...
   template "#{RESPONSE_FILE_PATH}" do
    ...
    execute "Install DB2 Express for BPM" do
      cwd node['im']['base']['path']
      command "imcl.exe input #{RESPONSE_FILE_PATH} -log C:/tmp/install_dbex9704_log.xml -acceptLicense"
      action :run
    end

else

  log "DB2 Express 9704 is already installed!" do
  level :info
  end
