...
if (!(File.directory?(node['mq7502']['base']['path'])))
	...
	windows_zipfile "#{node['mq7502']['config']['cache']}" do
	  source "#{node['mq7502']['config']['mount']}/#{node['mq7502']['zip']['file1']}"
	  action :unzip
	  overwrite true
	end
	...
	windows_package 'mq_7502 install' do
	source node['mq7502']['config']['cache'] + '/MSI/' + "IBM WebSphere MQ.msi"
