...
if (!(File.exists?(node['ihs8000']['base']['path'])&& File.directory?(node['ihs8000']['base']['path'])))
	...
	log "-------------DST Create the directory cache path #{node['ihs8000']['config']['mount']}-----------" do
		level :info
	end
	...
	log "-------------DST Create the directory cache path #{node['ihs8000']['config']['cache']}-----------" do
		level :info
	end
	...
	log "-------------DST Get the WAS zip 4 file -----------" do
	level :info
	end
	...
	log "-------------DST Unzip the IHS zip file 4 -----------" do
	   level :info
	end
	...
	template "#{node['ihs8000']['config']['cache']}/#{node['ihs8000']['response']['file']}" do
	...
	execute "Install IHS" do
	 cwd node['im']['base']
	 command "./imcl -acceptLicense -input #{node['ihs8000']['config']['cache']}/#{node['ihs8000']['response']['file']} -log #{node['ihs8000']['log']['file']}"
	 action :run
	end
  ...
  log "a verion of IBM HTTP Server already installed or a HTTP folder already exist!" do
  level :info
  end
