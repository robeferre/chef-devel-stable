...
#######################################################################
RESPONSE_FILE_PATH = "#{node['mq7503']['config']['cache']}/#{node['mq7503']['resp']['file']}"
...
#
##################################
log "-----DST - Get the Websphere MQ V7503 zip 1 file----"
...
windows_zipfile "#{node['mq7503']['config']['cache']}" do
  source "#{node['mq7503']['config']['mount']}/#{node['mq7503']['zip']['file1']}"
  action :unzip
  overwrite true
end
...
execute "installation of mq 7503" do
cwd node['mq7503']['config']['cache']
command "WS-MQ-7.5.0-FP0003.exe -f #{RESPONSE_FILE_PATH}"
action :run
end
