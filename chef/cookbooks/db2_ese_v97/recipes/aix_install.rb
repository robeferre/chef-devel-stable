...
response_path = node['db2']['cache'] + "/aix_server.rsp"
install_log_path = node['db2']['cache'] + "/silent_install.log"
license_path = node['db2']['adm']['path'] + "/db2ese.lic"
...
if (!(File.directory?(node['db2']['main']['path'])))
  ...
  execute "Activating the DB2 v9.7 license ..." do
    user "root"
    cwd node['db2']['adm']['path']
    command "./db2licm -a db2ese.lic"
    action :run
  end
  
  execute "Listing DB2 v9.7 license ..." do
    user "root"
    cwd node['db2']['adm']['path']
    command "./db2licm -l"
    action :run
