...
template RESPONSE_FILE_PATH do
    ...
    action :touch
     variables({
      ...
      :eclipseLocation     => "\'/usr/IBM/WebSphere/AppServer\'",
      :os                  => "\'aix\'",
      :arch                => "\'ppc\'",
      :ws                  => "\'motif\'",
...
execute "Create profile" do
  ...
  command "#{WAS_BASE_INSTALLATION}/manageprofiles.sh -create -templatePath /usr/IBM/WebSphere/AppServer/profileTemplates/default -profileName #{WAS_PROFILE_NAME}"
  action :run
end
...
if (!(File.readlines("/etc/inittab").grep(/startServer.sh server1/).any?))
  execute "Sending start server to /etc/inittab" do
      user "root"
      command "echo #{WAS_BASE_INSTALLATION}/startServer.sh server1 -profileName #{WAS_PROFILE_NAME} >> /etc/inittab"
      action :run
  end
end
